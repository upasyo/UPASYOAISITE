...
interface AdminDashboardProps {
  ...
  activeTab?: "settings" | "hero" | "research" | "projects" | "pubs" | "achievements" | "blog" | "knowledge" | "messages" | "links" | "animations" | "resume";
  setActiveTab?: (tab: "settings" | "hero" | "research" | "projects" | "pubs" | "achievements" | "blog" | "knowledge" | "messages" | "links" | "animations" | "resume") => void;
}
...
export default function AdminDashboard({ 
  ...
  const [localActiveTab, setLocalActiveTab] = useState<"settings" | "hero" | "research" | "projects" | "pubs" | "achievements" | "blog" | "knowledge" | "messages" | "links" | "animations" | "resume">("settings");
    ...
    <div className="bg-white dark:bg-brand-dark-card border border-zinc-200/60 dark:border-zinc-900 rounded-2xl shadow-2xl p-6 sm:p-8" id="admin-dashboard-panel">
      ...
      <div className="flex overflow-x-auto gap-1 border-b border-zinc-100 dark:border-zinc-800 pb-2 mb-6">
        {[
          ...
          { id: "resume", label: "CV_RESUME", icon: FileText },
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="border-t border-zinc-100 dark:border-zinc-800 pt-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* SPREADSHEET SYNC URL */}
                <div className="md:col-span-2 p-4 bg-emerald-50/60 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-xl space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <FileSpreadsheet className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                      <label className="block text-xs font-mono font-bold text-emerald-900 dark:text-emerald-300">
                        CONTACT FORM RESPONSES GOOGLE SPREADSHEET URL
                      </label>
                    </div>
                    {siteSettings.spreadsheetUrl && (
                      <a
                        href={siteSettings.spreadsheetUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1 text-[11px] font-mono font-semibold text-emerald-700 dark:text-emerald-400 hover:underline"
                      >
                  ...
                  <input
                    type="url"
                    value={siteSettings.spreadsheetUrl || ""}
                    onChange={(e) => setSiteSettings({ ...siteSettings, spreadsheetUrl: e.target.value })}
        ...
        {/* TAB 9: View Received Messages & Spreadsheet sync */}
        {activeTab === "messages" && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-200 dark:border-zinc-800 pb-3">
              <div>
                <h3 className="text-sm font-mono font-bold text-gray-700 dark:text-zinc-300 uppercase">
                  CONTACT FORM INBOX & GOOGLE SPREADSHEET SYNC
                </h3>
                <p className="text-xs text-gray-500 dark:text-zinc-400 font-mono mt-0.5">
                  Real-time visitor inquiries submitted via the Collaborative Router contact form.
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-mono font-bold bg-emerald-600 hover:bg-emerald-700 text-white transition-colors cursor-pointer shadow-xs"
                >
            ...
            {messages.length === 0 ? (
              <p className="text-xs text-gray-400 font-mono italic text-center p-8 bg-zinc-50 dark:bg-zinc-900 border border-dashed rounded-xl">
                No inbound submissions received yet. Inbound form submissions write to Firestore and can be reviewed here or in your Google Sheet.
              </p>
            ) : (
              <div className="space-y-4">
                {messages.map((msg) => (
        ...
        {/* TAB: EDIT FULL CV / RESUME */}
        {activeTab === "resume" && (
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-4">
              <div>
                <h3 className="text-sm font-mono font-bold text-gray-700 dark:text-zinc-300 uppercase">
                  CURRICULUM VITAE (CV / RESUME) FULL CMS COMPILER
                </h3>
                <p className="text-xs text-gray-500 font-mono mt-1">
                  Manage all research positions, competencies, academic credentials, publications, and awards. Changes reflect in real-time on the website and in printable PDF formats.
                </p>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center gap-2 bg-pink-600 hover:bg-pink-700 text-white font-bold px-4 py-2 rounded-xl text-xs cursor-pointer shadow-md transition-colors"
                >
                  <Save className="w-4 h-4" />
                  {isSavingResume ? "SAVING..." : "COMPILE & SAVE RESUME"}
                </button>
              </div>
            </div>

            {resumeSaveMsg && (
              <div className="p-3 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 rounded-xl text-xs font-mono">
                ✓ {resumeSaveMsg}
              </div>
            )}
            ...
            <div className="p-5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl space-y-4">
              <h4 className="text-xs font-mono font-bold text-brand-accent-pink uppercase">1. Basic Coordinates</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-xs font-bold dark:text-white"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">Professional Title</label>
                  <input
                    type="text"
                    value={resumeData.title}
                    onChange={(e) => setResumeData({ ...resumeData, title: e.target.value })}
                    className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-xs dark:text-white"
                  />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">Research Focus Line</label>
                  <input
                    type="text"
                    value={resumeData.focus}
                    onChange={(e) => setResumeData({ ...resumeData, focus: e.target.value })}
                    className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-xs dark:text-white"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">Email</label>
                  <input
                    type="email"
                    value={resumeData.email}
                    onChange={(e) => setResumeData({ ...resumeData, email: e.target.value })}
                    className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-xs font-mono dark:text-white"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">LinkedIn Profile</label>
                  <input
                    type="url"
            ...
            {/* Summary */}
            <div className="p-5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl space-y-2">
              <h4 className="text-xs font-mono font-bold text-brand-accent-pink uppercase">2. Executive Summary</h4>
              <textarea
                rows={4}
                value={resumeData.summary}
                onChange={(e) => setResumeData({ ...resumeData, summary: e.target.value })}
                className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3.5 py-2.5 text-xs dark:text-white leading-relaxed"
              />
            </div>
            ...
            <div className="p-5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl space-y-4">
              <div className="flex justify-between items-center">
                <h4 className="text-xs font-mono font-bold text-brand-accent-pink uppercase">3. Research & Work Experience ({resumeData.experience.length})</h4>
                <button
                  type="button"
                  onClick={() => setResumeData({
                    ...
                    experience: [
                      ...resumeData.experience,
                      {
                        ...
                        bullets: ["Designed variational quantum eigensolvers and Hamiltonian simulations."]
                      }
                    ]
                  })}
                  className="flex items-center gap-1 text-[11px] font-mono font-bold px-2.5 py-1 rounded bg-pink-600 text-white hover:bg-pink-700 cursor-pointer"
                >
              ...
              <div className="space-y-4">
                {resumeData.experience.map((exp, expIdx) => (
                  <div key={exp.id || expIdx} className="p-4 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="font-mono text-xs font-bold text-brand-accent-pink">Position #{expIdx + 1}</span>
                      <button
                        type="button"
                        onClick={() => setResumeData({
                          ...resumeData,
                          experience: resumeData.experience.filter((_, i) => i !== expIdx)
                        })}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">Role Title</label>
                        <input
                          type="text"
                          value={exp.role}
                          onChange={(e) => {
                          ...
                          className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded px-2.5 py-1.5 text-xs font-bold dark:text-white"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">Company / Lab</label>
                        <input
                          type="text"
                          value={exp.company}
                          onChange={(e) => {
                          ...
                          className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded px-2.5 py-1.5 text-xs dark:text-white"
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5 pt-2">
                      <div className="flex justify-between items-center">
                        <label className="text-[10px] font-mono text-zinc-400 uppercase">Contribution Bullets</label>
                        <button
                          type="button"
                          onClick={() => {
                            const updated = [...resumeData.experience];
                            updated[expIdx].bullets = [...updated[expIdx].bullets, "Novel research benchmark completed."];
                            setResumeData({ ...resumeData, experience: updated });
                          }}
                          className="text-[10px] font-mono font-bold text-pink-500 hover:underline flex items-center gap-0.5"
                        >
                      ...
                      {exp.bullets.map((b, bIdx) => (
                        <div key={bIdx} className="flex gap-2 items-center">
                          <input
                            ...
                            onChange={(e) => {
                              const updated = [...resumeData.experience];
                              const newB = [...updated[expIdx].bullets];
                              newB[bIdx] = e.target.value;
                              updated[expIdx].bullets = newB;
                              setResumeData({ ...resumeData, experience: updated });
                            }}
                            className="flex-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded px-2.5 py-1 text-xs dark:text-zinc-200"
                          />
                          <button
                            type="button"
                            onClick={() => {
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    await saveResumeData(resumeData);
                    setResumeSaveMsg("Resume successfully saved to Firestore!");
                    triggerStatus("success", "Resume saved and compiled successfully!");
                    setTimeout(() => setResumeSaveMsg(""), 3500);
                  } catch (e) {
                    console.error(e);
                ...
                disabled={isSavingResume}
                className="flex items-center gap-2 bg-pink-600 hover:bg-pink-700 text-white font-bold px-6 py-2.5 rounded-xl text-xs cursor-pointer shadow-md transition-colors"
              >
                <Save className="w-4 h-4" />
                {isSavingResume ? "SAVING..." : "COMPILE & SAVE RESUME"}
              </button>
