...
export default function App() {
  ...
  const loadProfileParameters = async (silent = false) => {
    ...
    try {
      ...
      // Automatically synchronize if database is currently populated with legacy non-quantum content
      const isLegacyContent = heroSnap && (
        !heroSnap.title?.toLowerCase().includes("quantum") && 
        !heroSnap.subtitle?.toLowerCase().includes("quantum")
      );
      if (isLegacyContent) {
        console.log("Synchronizing legacy profile with Quantum Computing Research dataset...");
        try {
          await syncQuantumSeedToDatabase();
          siteSnap = SEED_DATA.siteSettings;
          heroSnap = SEED_DATA.heroSection;
          aboutSnap = SEED_DATA.aboutSection;
          visionSnap = SEED_DATA.researchVision;
        } catch (syncErr) {
          console.warn("Auto sync quantum profile warning:", syncErr);
        }
      }
  ...
  const tickerItems = (siteSettings.tickerText || "QUANTUM INFORMATION SCIENCE, QUANTUM MACHINE LEARNING (QML), VARIATIONAL QUANTUM ALGORITHMS (VQE), SURFACE CODE ERROR CORRECTION, TENSOR NETWORK SIMULATIONS, QUANTUM-CLASSICAL CO-DESIGN, FAULT-TOLERANT QUANTUM COMPUTING")
    .split(",")
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-pink-200 dark:border-pink-900/60 bg-pink-50/60 dark:bg-pink-950/30 text-brand-accent-pink hover:bg-pink-100 dark:hover:bg-pink-900/50 text-[11px] font-mono uppercase font-bold transition-all cursor-pointer shadow-xs"
              title="View and Print Quantum Computing Resume / CV"
            >
          ...
          {selectedMobileMenu && (
            ...
            >
              <div className="flex flex-col gap-4 px-6 py-6 font-mono text-sm uppercase">
                ...
                <button
                  onClick={() => {
                    setSelectedMobileMenu(false);
                    setShowResumeModal(true);
                  }}
                  className="mt-2 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-pink-50 dark:bg-pink-950/40 text-brand-accent-pink border border-pink-200 dark:border-pink-900/60 font-bold tracking-wider cursor-pointer"
                >
                  <FileText className="w-4 h-4" />
                  <span>VIEW / PRINT CV & RESUME</span>
                </button>
              </div>
            </motion.div>
          )}
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 space-y-7 text-center lg:text-left">
                ...
                <div className="badge-quantum">
                  ...
                  <span>QUANTUM_COMPUTING_RESEARCH</span>
                </div>
                 ...
                 <h1 className="text-4xl md:text-6.5xl font-bold tracking-tight bg-gradient-to-br from-zinc-950 via-zinc-800 to-zinc-600 dark:from-white dark:via-zinc-100 dark:to-zinc-300 bg-clip-text text-transparent font-display leading-[1.05] select-text">
                  {localizedHero.title || "Quantum Computing & Advanced Neural Intelligence"}
                </h1>
                ...
                <div className="text-brand-accent-pink font-mono text-xs sm:text-sm font-semibold tracking-widest uppercase select-text h-[1.5em] flex items-center justify-center lg:justify-start">
                  <Typewriter 
                    texts={
                      language === "other"
                        ? [
                            localizedHero.subtitle || "Investigador en Computación Cuántica",
                            "Aprendizaje Automático Cuántico (QML)",
                            "Arquitecto Cuántico Variacional (VQE)",
                            "Teórico Cuántico Tolerante a Fallos",
                            "Especialista en Redes Tensoriales"
                          ]
                        : [
                            localizedHero.subtitle || "Quantum Computing Researcher & AI Scientist",
                            "Quantum Machine Learning (QML)",
                            "Variational Quantum Algorithms (VQE & QAOA)",
                            "Fault-Tolerant Surface Codes & QEC",
                            "Tensor Network Simulations (MPS/PEPS)"
                          ]
                    }
                    delay={80}
                    period={2500}
                ...
                <p className="text-zinc-700 dark:text-zinc-300 text-base md:text-lg max-w-xl leading-relaxed select-text font-light">
                  {localizedHero.description || "Pioneering the synthesis of Quantum Computing, Quantum Machine Learning (QML), and Fault-Tolerant Architectures. Developing hybrid quantum-classical algorithms, tensor network representations, and VQE eigensolvers to transcend classical computational limits."}
                </p>
                ...
                <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 pt-2">
                  {siteSettings?.buttons?.collab?.enabled !== false && (
                    ...
                    >
                      {t(siteSettings?.buttons?.collab?.name || "DEPLOY QUANTUM COLLAB")}
                  ...
                  {siteSettings?.buttons?.resume?.enabled !== false && (
                    ...
                    >
                      ...
                      {t(siteSettings?.buttons?.resume?.name || "VIEW_RESUME")}
                    </button>
                  )}
                  <ButtonLinks />
                </div>
            ...
            >
              ...
              <div className="lg:col-span-5 space-y-4 font-mono select-none">
                ...
                {/* SPREADSHEET FORM TRACKING LINK */}
                <div className="pt-2">
                  <a
                    ...
                    className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-800 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-all font-mono text-xs font-semibold shadow-xs"
                    id="contact-form-spreadsheet-link"
                    title="Open Form Submissions Spreadsheet"
                  >
                    <FileSpreadsheet className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                    <span>{t("FORM RESPONSES SPREADSHEET")}</span>
      ...
      <footer className="border-t border-zinc-100 dark:border-zinc-900 py-12 bg-zinc-50/50 dark:bg-brand-dark-base select-none">
        <div className="max-w-7xl mx-auto px-5 md:px-10 flex flex-col sm:flex-row items-center justify-between gap-6 font-mono text-xs text-zinc-500 dark:text-zinc-400">
          ...
          <div className="flex items-center gap-4 text-[10px]">
            {/* GOOGLE SPREADSHEET LINK AT BOTTOM */}
            <a 
              href={siteSettings.spreadsheetUrl || "https://docs.google.com/spreadsheets/d/1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms/edit?usp=sharing"} 
              target="_blank" 
              rel="noopener noreferrer" 
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-emerald-100/80 dark:bg-emerald-950/60 border border-emerald-300/80 dark:border-emerald-700/60 text-emerald-900 dark:text-emerald-300 hover:bg-emerald-200 dark:hover:bg-emerald-900/60 transition-colors font-mono font-bold text-[11px]" 
              title="Google Spreadsheet Form Responses"
              id="footer-google-spreadsheet-link"
            >
              <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-700 dark:text-emerald-400" />
              <span>FORM SPREADSHEET</span>
